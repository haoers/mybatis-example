...
public class Generator {
	...
	//①创建实体类
	public static ResultJson createEntity(String url,BasisInfo bi, boolean existDelFlg) {
		String fileUrl= getGeneratorFileUrl(url, bi.getEntityUrl(), bi.getEntityName(), ENTITY);
		return FreemarkerUtil.createFile(bi, "entity.ftl", fileUrl, existDelFlg);
	}
	
	//②创建DAO
	public static ResultJson createMapper(String url,BasisInfo bi, boolean existDelFlg) {
		String fileUrl= getGeneratorFileUrl(url, bi.getDaoUrl(), bi.getEntityName(), DAO);
		return FreemarkerUtil.createFile(bi, "dao.ftl", fileUrl, existDelFlg);
	}
	
	//③创建mapper配置文件
	public static ResultJson createMapperXml(String url,BasisInfo bi, boolean existDelFlg) {
		...
		bi.setAgile(agile);
		return FreemarkerUtil.createFile(bi, "mapper.ftl", fileUrl, existDelFlg);
	}
	
	//④创建SERVICE
	public static ResultJson createService(String url,BasisInfo bi, boolean existDelFlg) {
		String fileUrl= getGeneratorFileUrl(url, bi.getServiceUrl(), bi.getEntityName(), SERVICE);
		return FreemarkerUtil.createFile(bi, "service.ftl", fileUrl, existDelFlg);
	}
	
	//⑤创建SERVICE_IMPL
	public static ResultJson createServiceImpl(String url,BasisInfo bi, boolean existDelFlg) {
		String fileUrl= getGeneratorFileUrl(url, bi.getServiceImplUrl(), bi.getEntityName(), SERVICE_IMPL);
		return FreemarkerUtil.createFile(bi, "serviceImpl.ftl", fileUrl, existDelFlg);
	}
	
	//⑥创建CONTROLLER
	public static ResultJson createController(String url,BasisInfo bi, boolean existDelFlg) {
		createAbstractController( url, bi, existDelFlg); //保证父类一直存在
		String fileUrl= getGeneratorFileUrl(url, bi.getControllerUrl(), bi.getEntityName(), CONTROLLER);
		return FreemarkerUtil.createFile(bi, "controller.ftl", fileUrl, existDelFlg);
	}
	
	//⑦创建抽象的CONTROLLER
	public static ResultJson createAbstractController(String url,BasisInfo bi, boolean existDelFlg) {
		String fileUrl= getGeneratorFileUrl(url, bi.getAbstractControllerUrl(),"Abstract", CONTROLLER);
		return FreemarkerUtil.createFile(bi, "AbstractController.ftl", fileUrl, existDelFlg);
	}
	
	//⑧创建CONTROLLER
	public static ResultJson createSwaggerConfig(String url,BasisInfo bi, boolean existDelFlg) {
		String fileUrl= getGeneratorFileUrl(url,bi.getSwaggerConfigUrl(), "Swagger", SWAGGER_CONFIG);
		return FreemarkerUtil.createFile(bi, "SwaggerConfig.ftl", fileUrl, existDelFlg);
	}
	//生成文件路径和名字
