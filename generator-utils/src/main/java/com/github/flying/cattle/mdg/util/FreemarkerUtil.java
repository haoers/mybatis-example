...
public class FreemarkerUtil {
	...
	public static ResultJson createFile(BasisInfo dataModel, String templateName, String filePath, boolean existDelFlg) {
		...
		try {
			...
			if(!existDelFlg && file.exists()) {
				result.setCode(0);
				result.setMessage(fileName+"已存在");
				return result;
			}
			file.delete();
			file.createNewFile();
